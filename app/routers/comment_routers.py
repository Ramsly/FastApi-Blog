...
async def get_one_like(comment_id: int, db: Session = Depends(get_db)):
    return get_comment(db, comment_id)


@router.post('/create-comment', response_model=ResponseCommentSchema)
async def create_one_like(comment: ResponseCommentSchema, db: Session = Depends(get_db)):
    return create_comment(db, comment=comment)


@router.put('/update-comment/{comment_id}', response_model=ResponseCommentSchema)
async def update_one_like(comment_id: int, comment: ResponseCommentSchema, db: Session = Depends(get_db)):
...
async def delete_one_like(comment_id: int, db: Session = Depends(get_db)):
